/**
 * Group Crypto History — a developer tool that walks a Concord community's
 * cryptographic history epoch by epoch: given an invite link (or a community you
 * are already in), it derives each epoch's ConcordKeys, fetches that epoch's
 * plane events live, folds them into a CommunityState, and shows which addresses
 * rolled at each Refounding. Ported from applesauce's `concord/crypto-history`
 * example; adapted to ride the app's shared RelayPool and active account instead
 * of a standalone RelayPool + LoginView.
 */
import type { NostrEvent } from "applesauce-core/helpers/event";
import { ConcordRelayAuth, type CommunityState, type DecodedEvent, type InviteBundle, type JoinMaterial, type Role } from "applesauce-concord";
import {
  decodeWrap,
  decryptBundle,
  deriveConcordKeys,
...
// per-epoch stream keys we derive below. No ConcordClient — a manual, functional
// walk over the exported crypto core.
...
function Walker({
  ...
  // Load genesis once on mount; tear the auth drivers down on unmount. The ref
  // holds a single Subscription created at mount (never reassigned), so capturing
  // it for the cleanup is safe.
  useEffect(() => {
    aliveRef.current = true;
    const sub = driversSub.current;
...
/** Pick a source community — one you're already in, or a pasted invite link. */
function SourceEntry({ onPick }: { onPick: (material: JoinMaterial) => void }) {
  ...
  async function openInvite() {
    ...
    setError(null);
    try {
      onPick(await resolveInvite(invite.trim()));
    } catch (e) {
          ...
          <div className="flex flex-col gap-1">
            {communities.map((c) => (
              <button
                ...
                className="btn btn-ghost justify-start gap-2 font-medium"
      ...
      <section className="border border-base-300 rounded-box p-4 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <input
            ...
            onKeyDown={(e) => e.key === "Enter" && void openInvite()}
          />
          <button className="btn btn-primary" onClick={() => void openInvite()} disabled={!invite.trim() || resolving}>
  ...
  return (
    <>
      ...
      {material ? (
        ...
        <SourceEntry onPick={setMaterial} />
      )}
    </>
  );
}
