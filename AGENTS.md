...
- `scripts/*.mjs` — puppeteer-core browser drivers (need `pnpm dev` + `/usr/bin/google-chrome`): `drive.mjs`/`drive2.mjs` (single/two-user over real relays), `chan-cache.mjs` (channels survive reload with a dead relay), and `drive-auth.mjs` + `mock-auth-relay.mjs` (two users over a relay that requires NIP-42 AUTH to read).
